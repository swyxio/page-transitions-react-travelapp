      ...
      return <div>
        {users.map((user, i) => {
          return <div
            ...
            key={user.name}
            className={[user === selectedUser ? 'profile-photo' : 'profile-photo-secondary', `profile-${i}`].join(' ')}
            ref={`profile${i}`}
          >
        ...
        <button onClick={()=> alert('not implemented') || $.toggleFollow()} className={[following ?  'active-follow' : '', 'follow']} key="follow">
          {following ?
            <span >&#10004; Following</span> :
            <span >Follow</span>
          }
        ...
        <div onClick={()=> alert('not implemented') || $.addPlace()} className="side-icon" key="sideicon">
          {page === 'index' ?
...
function IconPlus() {
  return <g>
    <line className="line1" x1="0" x2="24" y1="12" y2="12" stroke="#333" stroke-linecap="round" stroke-width="3" />
    <line className="line2" x1="12" x2="12" y1="0" y2="24" stroke="#333" stroke-linecap="round" stroke-width="3" />
  </g>
}
